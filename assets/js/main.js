...
function toggleTabMenu() {
    ...
    if (menu && backdrop) {
        // Check if menu is hidden - menu is hidden if it has 'hidden' class OR '-translate-x-full' class
        // Also check display style as fallback
        const hasHiddenClass = menu.classList.contains('hidden');
        const hasTranslateClass = menu.classList.contains('-translate-x-full');
        const hasDisplayNone = menu.style.display === 'none';
        const isHidden = hasHiddenClass || hasTranslateClass || hasDisplayNone;
